...
public class AddUserAndContactAction implements Action
{
	public String execute(HttpServletRequest request, HttpServletResponse response)
	{
		...
		int userid=ud.login(username, password).getId();
		...
		Contact c=new Contact(rname, idtypeid, cardnum, sex, countryid, datebirth, mail, phone, touristtypeid, provinceid, schoolid, academy, classes, studentnum, schoollen, enterschoolyear, discountnum, startcityid, endcityid, userid, 1);
		boolean b=false;
		if(touristtypeid==3) 
		{
			b=cd.addContact(c);
		...
		if(b)
		{
			return "Login.do?loginname="+username+"&password="+password;
		}
		return "Register.do";
	}
